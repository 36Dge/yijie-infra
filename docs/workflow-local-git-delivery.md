...
| yijie-coze | `d6b79869b674b88cff5a8865dc640a2f65e8ff41` |

API/Coze/Desktop 消费者锁的 `source_commit` 必须一致指向上表 Contracts；canonical sync 的 `--check` 同时核对该提交内的源、生成物及来源锁。Coze origin 仍为上游，只允许使用用户明确指定的易界托管远端交付适配。
