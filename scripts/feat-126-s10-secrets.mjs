...
export function parseFeat126S10Secrets(text) {
  if (typeof text !== "string" || Buffer.byteLength(text) === 0 || Buffer.byteLength(text) > 4096) {
    throw new Error("FEAT-126 S10 secrets must be between 1 byte and 4 KiB");
  }
...
export async function validateFeat126S10Secrets(path) {
  ...
  return parseFeat126S10Secrets(await readFile(path, "utf8"));
}
